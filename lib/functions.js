...
const storeFileToIPFS = async (file_blob) => {
  ...
  // for font file
  // const file_ipfs = await nftstorage.storeDirectory([file])
...
const storeCSSToIPFS = async (name, originalname, file_ipfs) => {
  ...
  @font-face {
    font-family: '${name}';
    font-display: swap;
    src: url(https://${file_ipfs}.ipfs.nftstorage.link/${originalname});
  }
  `],
  originalname,
  {
...
const storeMetadataToIPFS = async (name, css_ipfs) => {
  ...
  const metadata = new Blob([`
  {
      ...
      "external_url": "https://${css_ipfs}.ipfs.nftstorage.link", 
...
const saveToTable = async (nftId, fontName) => {
  ...
  // Error: db query execution failed (code: SQLITE_no such column: undefined, msg: no such column: undefined)
